...
export ARG="TaskTrackAna702_merge.C(\"$1\",$2)"
aliroot -q -b -x $ARG
RET=$?
if [ "$RET" != "0" ];then
  echo "======== ERROR : TaskTrackAna702.C finished with NON zero code: $RET ========"
...
echo "======== TaskTrackAna702_merge.C finished with exit code: $? ========"
echo "############## memory after: ##############"
free -m
